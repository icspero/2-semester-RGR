...
bool isRus(char ch) {
    unsigned char uch = static_cast<unsigned char>(ch); // ïðèâîäèì ê òèïó unsigned char, ÷òîáû êîððåêòíî ðàáîòàòü ñ êîäîì ñèìâîëà
...
// Äîáàâëÿåì êîäîâîå ñëîâî â àëôàâèò
...
// Óäàëÿåì äóáëèêàòû ñèìâîëîâ
string remDup(string word) {
    string newWord = "";

    for (int i = 0; i < word.size(); i++) {
        if (newWord.find(word[i]) != string::npos) { // åñëè áóêâà óæå åñòü, òî ïðîïóñêàåì
            continue;
        }
        if (!isalpha(word[i]) && !isRus(word[i])) { // åñëè íå áóêâà è íå ðóññêàÿ áóêâà, òî ïðîïóñêàåì
            continue;
        }
        newWord += word[i];
    }
...
void codeWord(char action, const string& message) {
    string alphabet = "ÀÁÂÃÄÅ¨ÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäå¸æçèéêëìíîïðñòóôõö÷øùúûüýþÿABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
;
    if (action == '1') {
        char method;
        string codeWord;
